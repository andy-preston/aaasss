...
Deno.test("You can poke bytes", () => {
    ...
    const result = system.objectCode.poke([1, 2, 3, 4]);
...
Deno.test("Poked bytes are grouped in sets of 4", () => {
    const system = systemUnderTest();
    const result = system.objectCode.poke([1, 2, 3, 4, 5, 6]);
    expect(result.type).not.toBe("failures");
...
Deno.test("Poked bytes are padded to an even number", () => {
    {
        const system = systemUnderTest();
        const result = system.objectCode.poke([1, 2, 3]);
        expect(result.type).not.toBe("failures");
        expect(system.line.code).toEqual([[1, 2], [3, 0]]);
    } {
        const system = systemUnderTest();
        const result = system.objectCode.poke([1, 2, 3, 4, 5]);
        expect(result.type).not.toBe("failures");
...
Deno.test("You can also poke ASCII strings", () => {
    const system = systemUnderTest();
    const result = system.objectCode.poke(["Hello"]);
    expect(result.type).not.toBe("failures");
...
Deno.test("... or UTF-8 strings", () => {
    const system = systemUnderTest();
    const result = system.objectCode.poke(["ਕਿੱਦਾਂ"]);
    expect(result.type).not.toBe("failures");
    expect(system.line.code).toEqual([
...
Deno.test("... or a combination of bytes and strings", () => {
    const system = systemUnderTest();
    const result = system.objectCode.poke([1, 2, 3, 4, "Hello"]);
    expect(result.type).not.toBe("failures");
    expect(system.line.code).toEqual([
...
Deno.test("Poked numbers must be bytes (0-255)", () => {
    const system = systemUnderTest();
    const result = system.objectCode.poke([-1, 2, 300, 4]);
